  ...
  return (
    <section className="space-y-3">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-medium text-stone-800">Фотографии</h2>
      ...
      {loading ? (
        <p className="text-sm text-stone-600">Загрузка фотографий…</p>
      ) : photos.length === 0 && uploading.length === 0 ? (
        <p className="text-sm text-stone-500">Пока нет фотографий.</p>
      ) : (
        <ul className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          ...
          {uploading.map((file) => (
            <li
              key={file.name}
              className="flex aspect-square flex-col items-center justify-center rounded-xl border border-stone-200 bg-stone-50 p-3 text-center"
            >
              <span className="line-clamp-2 text-xs text-stone-700">{file.name}</span>
              {file.status === 'uploading' ? (
                <span className="mt-2 text-xs text-teal-800">Загрузка…</span>
              ) : (
      ...
      {photos.length < MAX_PHOTOS && (
        <label className="inline-flex cursor-pointer rounded-xl bg-teal-800 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-teal-900">
          Добавить фото
          <input
            ref={inputRef}
            type="file"
